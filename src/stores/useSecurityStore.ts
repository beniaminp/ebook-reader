 ...
 * Listens to Capacitor App lifecycle events to auto-lock on background.
 */
  ...
  persist(
    (set, get) => ({
      ...
      initialize: async () => {
        ...
        // Listen for app state changes (native only, but safe on web)
        try {
          ...
          // App plugin not available (web build without Capacitor) – ignore
        }
      },
      ...
      enableSecurity: async (lockType: LockType, credential?: string) => {
        ...
        await securityService.setCredential(credToStore, lockType as 'pin' | 'password' | 'biometric');
