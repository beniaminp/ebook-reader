 ...
 * Converts FB2 content to HTML for rendering in the scroll engine.
 */

import { XMLParser } from 'fast-xml-parser';

// FB2 XML namespaces
const FB2_NAMESPACES = {
...
const parserOptions = {
  ...
  removeNSPrefix: true,
  isArray: (name: string) => {
    // Only make these elements arrays, not title-info
...
class Fb2Service {
  ...
  convertFb2ToHtml(xmlContent: string): string {
    try {
      ...
      const sections = mainBody?.section || mainBody?.section || [];
   ...
   */
  private processSection(section: any, index: number): string {
    if (!section) return '';
