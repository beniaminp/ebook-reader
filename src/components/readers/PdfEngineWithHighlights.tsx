...
export const PdfEngineWithHighlights = forwardRef<ReaderEngineRef, PdfEngineWithHighlightsProps>((props, ref) => {
  const {
    pdfData,
    bookId,
    initialPage,
  ...
  const [selectedTextInfo, setSelectedTextInfo] = useState<{ text: string; rects: HighlightRect[] } | null>(null);
  ...
  const renderTextLayer = useCallback(async (pageNum: number, pageViewport: pdfjsLib.PageViewport, containerScale: number) => {
  ...
  // Update highlight
  const handleUpdateHighlight = useCallback((id: string, updates: { color?: string; note?: string }) => {
  ...
  const handleExportPdf = useCallback(async () => {
    try {
      ...
      await PdfService.exportAnnotatedPdf(
        pdfData,
        pdfHighlights,
        `annotated-${bookId}.pdf`
      );
    } catch (error) {
  ...
  return (
    <div ref={containerRef} style={{ position: 'relative', display: 'flex', justifyContent: 'center', alignItems: 'flex-start', minHeight: '100%', padding: '16px' }}>
      <PageTransition
        pageKey={currentPage}
