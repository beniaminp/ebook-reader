 ...
 *
 * Renders plain text files with theme integration, scroll-based reading,
...
export const TextReader: React.FC<TextReaderProps> = ({
  ...
  return (
    <IonPage>
      ...
      >
        <ReaderContainer>
          <div ref={innerRef} style={{ whiteSpace: 'pre-wrap', wordBreak: 'break-word' }}>
            {textContent}
          </div>
        </ReaderContainer>
      </IonContent>
      ...
      <IonModal isOpen={searchOpen} onDidDismiss={() => setSearchOpen(false)}>
        <IonHeader>
          <IonToolbar>
            <IonTitle>Search in Text</IonTitle>
            <IonButtons slot="end">
              ...
              <>
                ...
                <div style={{ maxHeight: '400px', overflowY: 'auto' }}>
                  {searchResults.map((result, idx) => (
                    ...
                    >
                      <IonLabel>
                        <p style={{ fontSize: '13px', whiteSpace: 'pre-wrap' }}>{result.context}</p>
                      </IonLabel>
                    </IonItem>
                  ))}
                </div>
