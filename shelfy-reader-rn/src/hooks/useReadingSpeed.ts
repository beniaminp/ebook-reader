 ...
 *
 * Reading speed is persisted to AsyncStorage so it improves over time
 * across sessions. The hook pauses tracking when the app goes to the
 * background or when the toolbar is open.
 *
 * React Native version: uses AsyncStorage instead of localStorage,
 * AppState instead of document.visibilitychange.
 */
...
// AsyncStorage key for persisting reading speed data
...
async function loadStoredData(): Promise<StoredSpeedData> {
  try {
...
async function saveStoredData(data: StoredSpeedData): Promise<void> {
  try {
...
export function useReadingSpeed(options: UseReadingSpeedOptions = {}): UseReadingSpeedReturn {
  ...
  const storedDataRef = useRef<StoredSpeedData>({ samples: [], averageWpm: DEFAULT_WPM });
  const [averageWpm, setAverageWpm] = useState(DEFAULT_WPM);
  ...
  // Load stored data on mount (async)
  useEffect(() => {
    let cancelled = false;
    loadStoredData().then((data) => {
      ...
      setTimeLeft((prev) => ({
        ...
        isReliable: data.samples.length >= 3,
      }));
  ...
  // Pause when app goes to background via AppState
  useEffect(() => {
  ...
  const calculateTimeLeft = useCallback(
    (progress: ReaderProgress): TimeLeftEstimate => {
      ...
      if (progress.total > 0 && progress.fraction < 1) {
        ...
        // If the engine provides time estimates, use them
