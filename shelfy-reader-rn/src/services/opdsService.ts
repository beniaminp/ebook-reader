 ...
 * - Uses standard fetch (no Capacitor/CapacitorHttp)
 * - Uses AsyncStorage instead of localStorage
 * - Keeps fast-xml-parser for OPDS feed parsing
 */
 ...
 * Uses a portable approach that works in React Native.
 */
function encodeBase64(str: string): string {
  // React Native has btoa available globally
...
export async function fetchOpdsFeed(
  ...
  // React Native has no CORS restrictions, so we can fetch directly
...
// ============================================================================
// CATALOG STORAGE (AsyncStorage)
// ============================================================================

const OPDS_CATALOGS_KEY = 'opds_catalogs';

export async function loadSavedCatalogs(): Promise<OpdsCatalog[]> {
  try {
    const stored = await AsyncStorage.getItem(OPDS_CATALOGS_KEY);
    if (stored) {
...
export async function saveCatalogs(catalogs: OpdsCatalog[]): Promise<void> {
  await AsyncStorage.setItem(OPDS_CATALOGS_KEY, JSON.stringify(catalogs));
}

export async function addCatalog(catalog: Omit<OpdsCatalog, 'id'>): Promise<OpdsCatalog> {
  const catalogs = await loadSavedCatalogs();
...
export async function updateCatalog(id: string, updates: Partial<Omit<OpdsCatalog, 'id'>>): Promise<void> {
  const catalogs = (await loadSavedCatalogs()).map((c) => (c.id === id ? { ...c, ...updates } : c));
  await saveCatalogs(catalogs);
}

export async function removeCatalog(id: string): Promise<void> {
  const catalogs = (await loadSavedCatalogs()).filter((c) => c.id !== id);
  await saveCatalogs(catalogs);
}
